...
void OnLogin(Player* player) override
{
	...
	player->m_richa.richard_importVariables_START(player->GetGUID());
	int a=0;
}
...
void OnChat(Player* player, uint32 type, uint32 lang, std::string& msg) override
{
	...
	takenInCharge = RichardClass::ExecuteCommand_richard_A(msg.c_str(), player);
	if ( takenInCharge ) return ;
	takenInCharge = RichardClass::ExecuteCommand_richard_B(msg.c_str(), player);
	if ( takenInCharge ) return ;
	takenInCharge = RichardClass::ExecuteCommand_richard_C(msg.c_str(), player);
	if ( takenInCharge ) return ;
	takenInCharge = RichardClass::ExecuteCommand_richard_D(msg.c_str(), player);
	if ( takenInCharge ) return ;
...
void OnPlayerCompleteQuest(Player* player____, Quest const* quest_id____) override
{
	...
	JE RETIRE LA PARTIE DU CODE QUI DONNE DES YOUHAICOIN QUAND ON COMPLETE UNE QUEST.
...
{
	// je sais pas si OnSave peux etre appelé sur plusieur joueurs en meme temps, mais dans le doute, un petit mutex, ca fait pas de mal
	std::lock_guard<std::mutex> lock(RichardClass::g_mutex_SavePlayerProtection); 
	...
	// normallement cette fonction sauvegarde les variables generales au serveur, et pas au joueur,
	// mais je la mets ici, car je vois pas ou la mettre ailleurs. c'est pas grave.
	// en plus on est protégé par un mutex.
	RichardClass::StaticRichardVariables_Save();
...
void OnCreatureKill(Player* killer, Creature* victim) override
{
	...
	// ca m'est arrivé une fois avec    victim.m_type=TOTEM_PASSIVE
	// je sais pas trop pourquoi, mais dans le doute je vais simplement dire que si killer null, alors on quitte direct
	if ( killer == nullptr )
	{
		return;
	}

	// je sais pas si ce cas peut arriver, je le mets juste par securité
	if ( victim == nullptr )
	{
		return;
	}
